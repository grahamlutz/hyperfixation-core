...
describe("a cloud hf new, all ten steps", () => {
  ...
  it("re-PATCHes the new password on a cold run, and creates nothing it can find", async () => {
    ...
    try {
      ...
      // hands a secret key back once. Nothing else is created a second time — the SOURCE_COMMIT
      // POST is a create only because this fixture's application lists no envs.
