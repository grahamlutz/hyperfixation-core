...
2. SSH and the Postgres loopback port: `ssh -T -o BatchMode=yes "$(jq -r .HF_SSH_HOST ~/.config/hf/config.json)" 'docker ps --format "{{.Names}}" | grep -i postgres; ss -ltn | grep 5432'` → the Postgres container name (your `HF_DB_HOST_INTERNAL` candidate) and a `127.0.0.1:5432` listener. **If no `5432` line appears the run fails at step 8** with `tunnel: could not reach Postgres on 127.0.0.1:5432` — Phase 0 left the port unexposed and the tunnel is the only transport that provisions (`provision-database.ts:80-86`). Publish the port on the box's loopback only (an explicit `127.0.0.1:5432:5432` mapping or a firewall rule; Coolify's "make public" binds all interfaces). X1 question 4.
3. `PGPASSWORD` over the tunnel: open `ssh -L 15432:127.0.0.1:5432 <host>` and run `psql "postgresql://postgres@127.0.0.1:15432/postgres" -c 'select 1'` → `1`.
