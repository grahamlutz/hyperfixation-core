...
9. **Restore-check needs a dump that `hf new` never triggers** (`backup_now: false`), and dumps may go to S3 only (Phase 0 configured S3; `--from-s3` is a refusal, `backup-source.ts:128-140`).
   - On the box the opposite was true: the schedule was registered `save_s3: true` with no `s3_storage_uuid`, so Coolify disabled the upload and kept the dump **local only**. Closed by `X1-fixes` (Finding below); a run that still prints the local-only WARNING leaves `--backup-dir` as the only path that has a dump at all.
