...
> **This has been run once, 2026-09-20.** `demo-app` is deployed. What the run produced — the evidence table, the four
> items still outstanding, and the chain of findings and their fixes — is in the
> [Phase 3 order's X1 exit section](hyperfixation-phase3-order-2026-09-19.md). The procedure below still stands for the
> next app; the gaps in section 7 say which are resolved.
...
| `HF_SMTP_URL` | a relay that actually delivers — sign-in is an emailed code. **Not SMTP on this box:** Hetzner blocks outbound 465 and `smtp.mx.cloudflare.net` speaks only 465, so X1 uses `cloudflare-email://` and the template sends over Cloudflare Email Sending's REST API (template #38) |
...
3. **Backup and restore.** `backup_now` is false (`backup.ts:43`), so trigger one: Coolify → the Postgres → Backups → the `hf_demo_app` schedule → Backup Now. Then `hf restore-check demo-app` → a table `table live restored verdict` with every `hf_*` row `ok` or `ok (drift +N)`, `N table(s) matched`, exit 0. The dump is a `pg_dump -Fc` file on the box's own disk at `/data/coolify/backups/databases/<team>/<name>-<uuid>/pg-dump-<db>-<epoch>.dmp` — for this run, `/data/coolify/backups/databases/root-team-0/shared-postgres-<uuid>/pg-dump-hf_demo_app-<epoch>.dmp`, about 93 KB — and `restore-check` finds it by `find`ing under `/data/coolify/backups` (`backup-source.ts:5`). `no hf_demo_app dump` means it landed elsewhere: `find /data/coolify -name '*hf_demo_app*'` on the box and pass `--backup-dir`.
   ...
   A live app keeps writing while the dump ages, so two counts that differ are not automatically a finding. `restore-check` carries a committed list of append-only `hf_*` tables — `hf_audit`, `hf_activity`, `hf_llm_call`, `hf_action_log`, `hf_approval`, `hf_score`, `hf_source_run`, `hf_run` (`restore-check.ts`, `APPEND_ONLY_TABLES`) — whose rows no code path deletes and no update can reduce the count of. On one of those, `restored <= live` reads `ok (drift +N)` and does not fail; `restored > live` is still a `mismatch`, because an append-only table cannot lose rows. Every other table, including one that merely looks append-only, is compared exactly. `--strict` drops the allowance and holds all 24 to exact counts — the check to run against a dump taken with the app stopped, where any drift at all is a finding. A dump older than 24 h prints a `WARN` line and exits 1 (as `hf doctor` does with its warnings): a comparison against stale data is not a passing check, so trigger a fresh backup rather than reading around the warning.
   ...
   > **Finding (verify 3, 2026-09-20): against a 1.3 h-old dump, `hf restore-check demo-app` called 9 of 24 tables mismatched, and none of them was.** `hf_run` 30 against 3, `hf_activity` 2 against 0, `hf_session` 3 against 0, `hf_verification` 1 live against 3 restored — every gap was the app working, or a session expiring, between the dump and the check; the same check against a fresh dump matched all 24. Fixed by the append-only list above, which reads the gap on a table that cannot lose rows as drift. `hf_session` and `hf_verification` stay exact — better-auth really does delete from both — so a dump that old still has to be re-taken, which is what the 24 h `WARN` now forces.
...
Markers added after the run: ✅ resolved, ⬜ still open.
