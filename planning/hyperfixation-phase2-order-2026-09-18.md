...
> `approval.<decision>`, and `record_type`/`record_id` fall back to `('hf_approval', id::text)`; the insert is
...
`records.archive()` still cancelling an approval assigned to someone else.
